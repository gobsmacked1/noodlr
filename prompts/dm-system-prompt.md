...
- **Recovery** - when the tracker corrupts (not if), correct the state and continue; one correction re-rails everything downstream. noodlr-main's advantage: it can rebuild the combat block from Foundry's actual combat tracker each turn and inject it as ground truth, making the module - not the model's last message - the source of truth.
